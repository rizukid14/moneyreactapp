          ...
          {items.map((item) => (
            <div key={item.id} className="p-3 rounded-xl bg-surface border border-outline-variant/40 space-y-2 text-xs">
              ...
              <div className="grid grid-cols-2 gap-2">
                ...
                <div>
                  <label className="text-[10px] font-bold text-outline uppercase block mb-1">Kategori</label>
                  <select
                    value={item.categoryId || ''}
                    onChange={e => handleUpdateItem(item.id, 'categoryId', e.target.value)}
                    className="w-full p-2 rounded-lg border border-outline-variant/50 bg-card-solid text-on-surface text-xs outline-none"
                  >
