...
const SettleUpModal: React.FC<SettleUpModalProps> = ({ isOpen, onClose, trip, expenses }) => {
  const { user, currencySymbol, addDebt } = useMoney();
  ...
  const handleMarkAsPaid = (t: typeof settlement.transactions[0]) => {
    ...
    showToast(`Penyelesaian dicatat di Debts!`, 'success');
  };
      ...
      <div className="modal-overlay" onClick={onClose} style={{ zIndex: 1100 }}>
        ...
        >
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px' }}>
            ...
            >
              {isSharing ? '...' : shareId ? <><CheckCircle2 size={18} /> Copy Link</> : <><Share2 size={18} /> Share Link</>}
            </button>
            <button 
              onClick={onClose}
