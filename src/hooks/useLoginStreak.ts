...
export function useLoginStreak() {
  ...
  useEffect(() => {
    ...
    const checkStreak = () => {
      ...
      const lastChecked = sessionStorage.getItem('lastStreakCheckDate');
      
      // Ensure we only run this once per day per session
      ...
      if (updated) {
        ...
        sessionStorage.setItem('lastStreakCheckDate', todayStr);
      } else if (lastLogin === todayStr) {
    ...
    // 1. Check immediately when component mounts or dependencies change
    checkStreak();

    // 2. Also check when the app is brought back to the foreground
